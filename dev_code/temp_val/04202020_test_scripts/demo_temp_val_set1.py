...
time python3 demo_temp_val_set1.py -p ~/temp_val/test/-2021-04-16-17-24-40_crop/ -ft jpg -min 20000 -tp ~/temp_val/mark_template/0420_marker.jpg
...
def comp_external_contour(orig,thresh):
    ...
    for c in contours:
        ...
        if w>img_width*0.01 and h>img_height*0.01:
            ...
            boundary = [0.5454, 0.5613, 0.5880, 
            0.6109, 0.6654, 0.6865,
            0.761358, 0.798785, 0.816492]
            
                        
            project_val = [0.632042534454521, 0.850837220128791, 0.874526261697102,
                        0.75687434022049, 0.834619535106866, 0.821430086931889, 
                        0.74034274835652, 0.857772049737514, 0.774673393446344]

            
            if index < boundary[1]:
                index = project_val[0]
                
            elif index > boundary[0] and index < boundary[2]:
                index = project_val[1]
                
            elif index > boundary[1] and index < boundary[3]:
                index = project_val[2]
                
            elif index > boundary[2] and index < boundary[4]:
                #print("index  = {}... \n".format(index))
                index = project_val[3]
                
            elif index > boundary[3] and index < boundary[5]:
                index = project_val[4]
                
            elif index > boundary[4] and index < boundary[6]:
                index = project_val[5]
                
            elif index > boundary[5] and index < boundary[7]:
                index = project_val[6]
                
            elif index > boundary[6] and index < boundary[8]:
                index = project_val[7]
                
            elif index > boundary[7]:
                index = project_val[8]
